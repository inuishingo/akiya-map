...
## survey_grids（調査グリッド）スキーマ

固定250mメッシュの調査済みマス。`index.html` の `toggleGridCell` が set/delete。doc ID = `G_{row}_{col}`。
- メッシュ定義：原点 `lat0=35.00 / lng0=136.80`、`sizeM=250`、`refLat=35.2`、`gridVersion="v1-250m"`。`gridCellCenter(row,col)` で中心復元可。
- フィールド：`gridId / row / col / surveyedDate / surveyedBy(email) / recordedAt / branch / gridVersion`。
- 「存在＝現周回で調査済み」。周回リセット(C-2)ではマスを**削除**する（薄く残さない・surveyRoundは持たせない）。
- 塗り色は担当者色（`staffColor(surveyedBy)` = `displayNames[email].order` → STAFF_PALETTE）。ピンのラベル色とは別系統。

## 【C-3 保留・申し送り】survey_grids への areaCode 付与

C-2（エリア別の周回リセット）にはグリッドの市区町村判定が要る。当初 ZENRIN reverse（ピンと同一経路）で
セル中心を判定しようとしたが**却下**。理由と方針：
- **ZENRIN reverse ではグリッドの areaCode を判定できない**。セル中心が道路・河川・畑・水面に落ちると
  住所が返らず、実測で **1045件中 439件(42%)が null**（四半点サンプリングでも約26%残存）。再試行でも復活せず**真の hit:0**。
  原因は「住所で市区町村を判定していること」——道路や河川に住所は無いが行政区域は存在する。
- **採用方針（別セッションで実装）**：国土数値情報 行政区域データ（N03）の市区町村ポリゴンに対し、
  セル中心を **point-in-polygon 判定**。null が原理的に発生せず ZENRIN も叩かない（無課金）。
  着手時にまず **N03 の JIS5桁コードがピン側 areaCode（ZENRIN `address_code2+3`）と同一体系**であることを実証する
  （名古屋市港区=23111 / 四日市市=24202 / 菰野町=24341 / 京都市伏見区=26109）。
- dry-run 実績（2026-07・参考CSV `C:\Users\user\Documents\grid_backfill_dryrun.csv`）：areaCode取得606 / null439。
  整合性ゲート 49/50一致。不一致1件は市境（`G_5_-51`：中心=川越町24344 / セル内のピン=四日市市24202）で仕様どおりの正しい挙動
  （グリッドはセル中心で帰属、ピンはピン座標で帰属）。
- C-3で入れたグリッド側コード（`resolveGridArea`・reverse後追い保存・`grid_area_cache` コレクション/rules）は**ロールバック済み**。
  ピン側の `fetchReverseItem` / `areaFromReverseItem` 共有ヘルパーと `reverseGeocode` の pendingAreaCode リセットは改善として残置。
- C-2 でグリッドを削除する際、将来 areaCode キャッシュを導入する場合はそれを一緒に消さないこと（消すと再判定コスト再発）。
